...
private:
  ...
  string rawInputBasicCheatTwoInput =   "...\n"
                                       ".#.\n"
                                       ".#.\n"
                                       "S#E\n"
                                       "###\n";

  string rawInputBasicCheatThreeInput = "#S....\n"
                                        "#####.\n"
                                        "#E....\n";
...
protected:
  ...
  vector<string> inputBasicTwoCheat;
  vector<string> inputBasicThreeCheat;

  // Setup after each
  void SetUp() override {
    inputBasic = InputUtils::convertToVector(rawInputBasic);
    inputBasicCheat = InputUtils::convertToVector(rawInputBasicCheatInput);
    inputBasicTwoCheat = InputUtils::convertToVector(rawInputBasicCheatTwoInput);
    inputBasicThreeCheat = InputUtils::convertToVector(rawInputBasicCheatThreeInput);
  }
...
TEST_F(RaceTrackTests, ShouldFindBasicCheatSourceLocationsZero) {
  // Given
  /*
  ....
  .##.
  ...
  // When
  vector<std::pair<pathfinding::MazeTile *, core::Orientation>> result = raceTrack.findAllCheatSourceOptions();
  // Then
  ASSERT_EQ(0, result.size());
}

TEST_F(RaceTrackTests, ShouldFindBasicCheatSourceLocationsTwo) {
  // Given
  /*
  ...
  .#.
  ...
  ###
   */
  RaceTrack raceTrack(inputBasicTwoCheat);
  // When
  vector<std::pair<pathfinding::MazeTile *, core::Orientation>> result = raceTrack.findAllCheatSourceOptions();
  // Then
...
TEST_F(RaceTrackTests, ShouldFindBasicCheatSourceLocationsThree) {
  // Given
  RaceTrack raceTrack(inputBasicThreeCheat);
  // When
  vector<std::pair<pathfinding::MazeTile *, core::Orientation>> result = raceTrack.findAllCheatSourceOptions();
  // Then
